 ...
 * world frame using the robot model state. Intended for automated testing and
 * benchmarking of perception, planning, and manipulation pipelines.
 *
 * Features:
...
/**
 * @brief Retrieve all available block models in the models directory.
 * @return List of model names with valid SDF files.
 */
...
/**
 * @brief Check if a point is inside the predefined NO-GO zone.
 * @param x X coordinate in base_link frame.
 * @param y Y coordinate in base_link frame.
 * @return True if inside NO-GO zone, False otherwise.
 */
 ...
 *
 * Respects table boundaries, radial reachability constraints, and NO-GO regions.
 * Retries multiple times before returning a fallback position.
 *
 * @return Valid (x, y) tuple in base_link frame.
 */
def sample_xy_base():
    for _ in range(2000):
 ...
 *
 * Uses the current robot pose in Gazebo to map coordinates consistently.
 *
 * @param get_state_srv Gazebo service proxy for GetModelState.
 * @param x_b X in base_link frame.
 * @param y_b Y in base_link frame.
 * @param z_b Z in base_link frame.
 * @param yaw_b Yaw in base_link frame.
 * @return Pose in Gazebo world frame.
 */
...
/**
 * @brief Convert a pose from Gazebo world frame to base_link frame.
 * @param get_state_srv Gazebo service proxy for GetModelState.
 * @param x_w X in world frame.
 * @param y_w Y in world frame.
 * @param z_w Z in world frame.
 * @param yaw_w Yaw in world frame.
 * @return Tuple (x_b, y_b, z_b, yaw_b) in base_link frame.
 */
 ...
 *
 * Ensures a minimum distance from previously spawned objects.
 * Converts the valid base_link pose to world frame for Gazebo spawning.
 *
 * @param existing_xy_base List of occupied (x, y) positions in base_link.
 * @param get_state_srv Gazebo service proxy for GetModelState.
 * @return Tuple: (Pose in world frame, (x_b, y_b) in base_link)
 */
...
# FIX FISICA GAZEBO
/**
 * @brief Override Gazebo physics parameters for stable block spawning.
 *
 * Modifies ODE physics settings to improve contact stability and reduce
 * numerical artifacts when spawning small objects on planar surfaces.
 */
 ...
 * and spawns them at valid poses in the simulation.
 */
def main():
